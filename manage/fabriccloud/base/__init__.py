...
def provisionEBS(format=False):
    '''
    :param format: formats the ebs volumes, use with care
    :return:
    '''


    if not exists('/mongo'):
        sudo('mkdir -m 000 /mongo')

        if format:
            sudo('mkfs.ext4 /dev/xvdc')

        sudo('mount /dev/xvdc /mongo')

    if not exists('/assetstore'):

        sudo('mkdir -m 000 /assetstore')

        if format:
            sudo('mkfs.ext4 /dev/xvdd')

        sudo('mount /dev/xvdd /assetstore')
...
def _installMongoDB(verbose=False):
    ...
    with settings(warn_only=True):
        ...
        path = os.path.join(os.path.abspath(os.curdir), 'config/mongod.girder.conf')
        put(path)

        _remote_sudo('mv -v /etc/mongod.conf /etc/mongod.conf.old', True)
        _remote_sudo('mv -v ./mongod.girder.conf /etc/mongod.conf', True)
...
def installGirder():
    ...
    appname = 'girder'
    giturl = 'https://github.com/girder/girder.git'
    ...
    if exists(os.path.join(app_path, appname)):
        ...
        # run git clone
        clonestring = 'git clone %s %s/%s' % (giturl, app_path, appname)
        _remote_cmd(clonestring, verbose=True)
...
def configureGirder():
    ...
    with settings(warn_only=True):
        ...
        girder_path = os.path.join(app_path, appname)

        nginx_config_path = os.path.join(os.path.abspath(os.curdir), 'config/girder_nginx.saf')
        supervisor_config_path = os.path.join(os.path.abspath(os.curdir), 'config/girder_supervisor.saf')

        put(nginx_config_path, girder_path + '/nginx.saf')
        put(supervisor_config_path, girder_path + '/supervisor.saf')
...
def installSSLKeys():
    ...
    with settings(warn_only=True):
        ...
        key_path = os.path.join(os.path.abspath(os.curdir),'../..','uda-admin/keys/uda2study_com.key')
        sslbundle_path = os.path.join(os.path.abspath(os.curdir),'../..','uda-admin/sslcert/ssl-bundle.crt')

        put(key_path, girder_path)
        put(sslbundle_path, girder_path)

        sudo('sudo mv -v /home/ubuntu/applications/girder/ssl-bundle.crt /etc/ssl/certs/ssl-bundle.crt')
        sudo('mv -v /home/ubuntu/applications/girder/uda2study_com.key /etc/ssl/private/uda2study_com.key')
